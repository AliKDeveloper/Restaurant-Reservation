<div class="w-full max-w-md mx-auto p-4 bg-white border border-gray-200 rounded-lg shadow sm:p-6 md:p-8 dark:bg-gray-800 dark:border-gray-700">
    <form action="{{ route('table.addTables') }}" method="post">
        ...
        <div class="space-y-6">
            ...
            <div>
                <label for="secrete_key" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">
                    {{ __('table/table.secrete_key') }}
                </label>

                <input type="password" name="secrete_key" id="secrete_key" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white" placeholder="{{ __('table/table.secrete_key_placeholder') }}">
            </div>

            <span class="text-sm text-red-600 dark:text-red-400"> {{ __('table/table.secrete_key_note') }} </span>
